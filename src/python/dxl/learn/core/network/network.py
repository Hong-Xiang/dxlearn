...
class Network(Model):
    ...
    def __init__(self, name='network', inputs=None, submodels=None, config=None, info=None,
                 *, objectives=None, trainers=None, summaries=None, saver=None,
                 add_trainers=None,
                 add_saver=None):
        """
        ...
        must **NOT** create new tensor in this function since it will be called
    ...
    def _fetech_tensor_maybe_in_dict(self, group_name, name=None):
        ...
        raise ValueError(
            "Invalid feteching tensor: {}/{}.".format(group_name, name))

    def train(self, name=None, feeds=None):
        trainer = self._fetech_tensor_maybe_in_dict(
            self.KEYS.TENSOR.TRAINERS, name)
        trainer.train(feeds)

    def inference(self, name=None, feeds=None):
        t = self._fetech_tensor_maybe_in_dict(
            self.KEYS.TENSOR.INFERNECES, name)
