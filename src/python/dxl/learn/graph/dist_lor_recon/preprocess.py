...
def partition_lors(lors: np.ndarray):
  ...
  # zlors = lors[z_mask, :]
  xlors = lors[np.array(
      [np.intersect1d(np.where(x_d >= y_d), np.where(x_d >= z_d))])].reshape(
          (-1, dim_size1))
  ylors = lors[np.array(
      [np.intersect1d(np.where(y_d > x_d), np.where(y_d >= z_d))])].reshape(
          (-1, dim_size1))
  zlors = lors[np.array(
      [np.intersect1d(np.where(z_d > x_d), np.where(z_d > y_d))])].reshape(
          (-1, dim_size1))
...
def swap_points(lors: np.ndarray, axis: int):
  ...
  swaped_nega = np.hstack((np.array(negative[:, 3:6]),
                           np.array(negative[:, 0:3]), np.array(
                               negative[:, 6:])))
