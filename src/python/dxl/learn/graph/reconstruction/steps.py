...
from .utils import ImageInfo, print_tensor, DataInfo, load_data, logger, constant_tensor
...
def task_init(job, task, config=None):
  task = DistributeTask(load_cluster_configs(config))
  return task
...
def create_worker_graphs(task: DistributeTask, image_info,
  ...
  for i in range(task.nb_workers()):
    ...
    worker_graphs = [WorkerGraphLOR(
        task.master_graph,
        image_info,
        lors_
    )]
    with DistributeGraphInfo('datas', 'data_scope', None,
                             ThisHost.host()).variable_scope():
      emap = constant_tensor(emap)
      lors = {a: constant_tensor(lors[a]) for a in lors}
    local_graphs = [
        LocalGraph(global_graph, h, emap, lors, image_info) for h in hosts
    ]
    for lg in local_graphs:
      lg.make_recon_step()
    logger.info("Local graph created.")
  return local_graph
...
def init_step(global_graph, hosts):
  ...
  global_init_barrier = Barrier('global_init', [master_host], hosts,
                                [[TK.INIT_OP]])
