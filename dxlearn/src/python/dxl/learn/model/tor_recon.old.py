...
import numpy as np
op = tf.load_op_library('/home/hongxwing/Downloads/tensorflow/bazel-bin/tensorflow/core/user_ops/pet.so')
...
class ReconStep(Model):
    ...
    def kernel(self, inputs):
        ...
        centery = center
        sizey = size
        py = op.projection(lors=ylors, image=imgy, grid=gridy,
                           center=centery, size=sizey, kernel_width=kernel_width, model=model)

        bpy = op.backprojection(image=imgy, grid=gridy, lors=ylors,
