...
def create_2_layers_dnn_for_mnist():
    class DNNWith2Layers(Network):
        def kernel(self, inputs):
            ...
            h = self.subgraph('layer1', lambda g: Dense(h, self.config('hidden_units')[1]), activation=ReLU)(h)
            label = inputs['y']
            y_ = self.subgraph('layer1', lambda g: Dense(h, self.config('hidden_units')[2]))(h)
